...
function updateUI(compress: boolean, gain: number): void {
    ...
    // 更新狀態文字
    statusText.textContent = compress ? `Compression ON (Gain: ${gain.toFixed(1)})` : 'Compression OFF';
}
...
// 通知所有支援的 tab
async function notifyTabs(message: CompressionMessage | GainMessage): Promise<void> {
    try {
...
async function handleToggleClick(): Promise<void> {
    try {
        ...
        statusText.textContent = newCompress ? 'Compression enabled' : 'Compression disabled';
    } catch (error) {
